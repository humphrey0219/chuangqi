 ...
 *==================================*/
+function($) {
	$.fn.citys = function(options, callback) {
		//定义参数
		var _options = $.extend({
        ...
        $selArea.on('change',function(){
            var _thisVal = $(this).find('option:selected').data('code') || $(this).find('option').eq(0).data('code');
            changeArea(_thisVal, true);  
            result.county = _thisVal
            result.finish =true
            callback(result)         
        })
        ...
        function changeArea(val, finish){
