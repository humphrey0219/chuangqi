...
(function ($)
{
    ...
    //重写js的toJSON方法，使时间对象json格式化之后依然保持本地时间
...
(function ($)
{
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _bulidGroupTableHtml: function (altering, allowDelete)
        {
            ...
            tableHtmlArr.push('<tr class="l-filter-rowlast"><td class="l-filter-rowlastcell" align="center" colSpan="4">');
            //and or
            ...
            //add group
//            tableHtmlArr.push('<input type="button" value="' + p.strings['addgroup'] + '" class="addgroup l-button">');
            //add rule
            tableHtmlArr.push('<input type="button" value="' + p.strings['addrule'] + '" class="addrule l-button">');
//            if (allowDelete)
//                tableHtmlArr.push('<input type="button" value="' + p.strings['deletegroup'] + '" class="deletegroup l-button">');
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        width: 'auto',                          //宽度值
        height: 'auto',                          //宽度值
        columnWidth: 45,                      //默认列宽度
        minColumnWidth: 45,						//默认列最小宽度
        ...
        sortFirstFlag:1,                    //是否第一次排序，用于排序后分页
        sortnameFirstParmName: null,
        sortorderFirstParmName: null,
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' value='1' maxlength='4' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadData: function (loadDataParm)
        {
            ...
            if (p.dataAction == "server")
            {
            	if (p.sortName)
                {
            		if(p.sortName != p.sortnameFirstParmName || p.sortOrder != p.sortorderFirstParmName){
            			p.sortFirstFlag = 1;
            			p.sortnameFirstParmName = p.sortName;
            			p.sortorderFirstParmName = p.sortOrder;
            		}else{
            			p.sortFirstFlag = 2;
            		}
            		if( p.sortFirstFlag == 1 ){
            			p.newPage = 1;
            		}
        ...
        */
        ...
        changePage: function (ctype)
        {
            ...
            if (p.dataAction != "local" && g.isDataChanged && false)
                return false;
        ...
        _showData: function ()
        {
            ...
            if (!data || !data.length)
            {
                ...
                $("<div style='margin-top:5px;margin-left:25px;'>"+p.noRecordMessage+"</div>").addClass("l-grid-body-inner").appendTo(g.gridbody).css({
                    width: 200,
                    height: 26
                });
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            htmlarr.push('min-height:' + p.rowHeight + 'px;line-height:' + p.rowHeight + 'px;');
